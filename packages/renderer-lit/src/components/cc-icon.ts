...
import type { IconComponent, DataModel } from '@claude-canvas/core';

// Simple inline SVG icons - a subset of common icons
// Using inline SVGs for simplicity (no external icon library dependency)
...
export class CcIcon extends LitElement {
  ...
  render() {
    ...
    const size = this.component.size ?? 24;
    const color = this.component.color ?? 'currentColor';
    const path = ICONS[name];

    if (!path) {
      console.warn(`Unknown icon: ${name}`);
      return html`<span style="width: ${size}px; height: ${size}px;">?</span>`;
    }
